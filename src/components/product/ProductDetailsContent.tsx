...
const SHOE_IMAGES = [detailsShoe1, detailsShoe2, detailsShoe3, shoe1];
...
export default function ProductDetailsContent({ productId }: ProductDetailsContentProps) {
        ...
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-12 animate-pulse">
          <div className="lg:col-span-8">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="aspect-square rounded-[2rem] bg-zinc-300" />
              ))}
            </div>
          </div>
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="h-8 w-24 bg-zinc-300 rounded-md" />
            <div className="h-12 w-full bg-zinc-300 rounded-md" />
            <div className="h-10 w-32 bg-zinc-300 rounded-md" />
            <div className="flex flex-col gap-4 pt-8">
              <div className="h-14 w-full bg-zinc-300 rounded-xl" />
              <div className="h-14 w-full bg-zinc-300 rounded-xl" />
            </div>
          </div>
        </div>
  ...
  // Map API images and fallback to dummy ones for the 2x2 grid design
  const displayImages = [...(product.images || [])];
  while (displayImages.length < 4) {
    displayImages.push(SHOE_IMAGES[displayImages.length % 4] as any);
  }
  const finalImages = displayImages.slice(0, 4);
          ...
          <div className="lg:col-span-8">
            ...
            <div className="hidden lg:grid grid-cols-2 gap-4">
              {finalImages.map((img, idx) => {
                ...
                // Specific corner rounding for the 2x2 grid look
                const cornerClass = 
                  idx === 0 ? "rounded-tl-[4rem]" : 
                  idx === 1 ? "rounded-tr-[4rem]" : 
                  idx === 2 ? "rounded-bl-[4rem]" : 
                  idx === 3 ? "rounded-br-[4rem]" : "";

                return (
                  <div 
                    key={idx} 
                    className={`relative aspect-square overflow-hidden max-h-[510px] bg-[#ECEEF0] ${cornerClass}`}
                  >
                    <Image
                ...
                {finalImages.map((img, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative aspect-square overflow-hidden rounded-lg bg-[#ECEEF0] transition-all ${
                        activeIndex === idx ? "ring-2 ring-primary ring-offset-2 scale-95" : "opacity-70"
                      }`}
                    >
                      <Image
                        src={isRemote ? getDirectImageUrl(img as string) : img}
